...
Afin d'uniformiser les projets et les conditions de développements, des template sont utilisés. Un template est la réplique d'un dépôt Git contenant au préalable une base de code déjà existant. Le template est donc différent en fonction du projets qui est créé. Pour l'instant il y a 6 template différents :
\begin{itemize}
...
\end{itemize}
Pour des raisons de sécurités, les donnée comme les codes permettant l'authentifications à la base de donnée ainsi que d'autres éléments sensibles sont sauvegarder dans les variables d'environnement des serveurs sur lesquels sont déployés les

La création d'un projet commence donc par la création d'un dépôt git associé au projet sur GitLab. De plus GitLab permet l'enregistrement de variables d'environnements grâce à son module CI/CD (Continuous Integration / Continuous Deployment). Un script permet de créer le dépôt git avec le template choisi et les variables d'environnement associés au projet (cf \textbf{Figure \ref{fig:varenv} en Annexe 9.4}) automatiquement.
