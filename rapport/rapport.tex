...
Finalement, une fois que l'application/une fonctionnalité de l'application est développée. Le consultant pousse ses modifications sur le dépôt, et c'est ici que le travaille du consultant s'arrête. C'est cette action qui va enclencher tout le processus de déploiement qui est donc continue. La figure suivante (\textbf{figure \cite{fig:res}})résume les différentes étapes du développement en local.
...
GitLab permet l'enregistrement de variables d'environnements grâce à son module CI/CD. Il les intègre directement dans la pipeline, c'est comme cela qu'on retrouve les variables d'environnements. 
